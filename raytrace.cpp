		                      ...
		                      Update_info* info)
{
	...
	double c =  cen_x * cen_x - 2 * cen_x * x1 + cen_z * cen_z - 
		2 * cen_z * z1 + x1 * x1 + z1 * z1 - r * r;
	...
	double end_cap_t = end_cap_t1;
	if (end_cap_t1 > end_cap_t2)
	...
	if (end_cap_t > 0)
	{
		...
		double check = (cen_x - intersect.X()) * (cen_x - intersect.X())
				            +(cen_z - intersect.Z()) * (cen_z - intersect.Z())
										- r * r;
		if (check < 0 && (end_cap_t < info -> t || info -> t < 0))
		{
			info -> t = end_cap_t;
			info -> position = intersect;
		        ...
		        int max_depth, int cur_depth, bool hard_shadow)
{
	...
	for (int i = 0; i < NLights; i++)
	{
		...
		if (cur_depth < max_depth)
		{
			...
			R3Intersect reflect_info = Intersect_scene(reflect_ray, scene);
		...
		if (hard_shadow)
		{
			...
			ret_info = Traverse_scene(*shadow, scene -> root, cur_best, shadow->Start());


			if(ret_info.info.t < DBL_MAX)
			{
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			if (info.hit)
			{
				...
				R3Rgb radiance = ComputeRadiance(scene, ray, &info, u, v, 0, true);
